...
std::vector<uint64_t> EthernetInterface::recieve_burst_single_packet(int timeout_sec, int timeout_us )
{
    int numbytes;
    ...
    if(retval > 0)
    {
        ...
        int wordsRead = (numbytes-2)/8;
        data.resize(wordsRead);
        
        for (int i = 0; i < wordsRead; ++i)
        {
            memcpy((void*)(data.data()+i), (void*)&buff_[TX_DATA_OFFSET_ + 8*i], 8);
        }
    }
    else if(retval == 0)
    {
